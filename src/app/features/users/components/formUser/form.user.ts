...
export class FormUser {
  ...
  userForm = new FormGroup({
    ...
    role: new FormControl<UserRole>({value: UserRole.STUDENT || UserRole.PROFESSOR, disabled: true}, Validators.required)
  })

  //Método para confirmar si la contraseña y la confirmación de la contraseña coinciden
  ...
  constructor(private route: Router,
    ...
    ngOnInit():void{
      ...
      //Estado del botón de Rol según el rol del usuario autenticado
    ...
    onUpdate(){
      ...
      const userFormData : User = this.userForm.value as User; //Usuario del formulario
      const confirmPassword = this.userForm.get('confirmPassword')?.value;
      ...
      if(!this.userForm.value.password){
        ...
        this.error.set('Debes completar todos los campos correctamente.');
        this.loading.set(false);
        return;
      }
      ...
      if(this.userForm.value.password && 
        ...
        this.userForm.get('password')?.markAsTouched();
        this.error.set('La contraseña debe tener mínimo 8 carácteres.');
        this.loading.set(false);
        return;
      }
      ...
      if(this.userForm.value.confirmPassword && 
        this.userForm.value.confirmPassword !== this.userForm.value.password
      ){
        this.error.set('La contraseña no coincide.');
        this.loading.set(false)
        return;
      }

      const resultUpdate = this.userService.updateUser(id!, userFormData)
